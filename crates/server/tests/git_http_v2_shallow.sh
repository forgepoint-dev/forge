...
for i in {1..5}; do echo $i >> "$TMP/log"; git -C "$TMP" add log; git -C "$TMP" -c user.email=t@e -c user.name=t commit -m "c$i" >/dev/null; done
git -C "$TMP" branch -M main >/dev/null
git -C "$TMP" remote add origin "$REPOS_DIR/$REPO_NAME.git"
git -C "$TMP" push origin main >/dev/null

DEST=$(mktemp -d)
git -c protocol.version=2 clone --depth=1 "$SERVER_URL/$REPO_NAME" "$DEST/repo" >/dev/null 2>&1
[[ -d "$DEST/repo/.git" ]] || { echo "shallow clone failed"; exit 1; }
echo "ok"
