...
impl ExtensionManager {
    ...
    fn extract_extension_name(wasm_path: &PathBuf) -> Result<String> {
        ...
        if wasm_path.extension().and_then(|s| s.to_str()) != Some("wasm") {
            return Err(anyhow::anyhow!("File is not a WASM module: {:?}", wasm_path));
        }
    ...
    async fn load_extension(&self, name: &str, wasm_path: &PathBuf) -> Result<Extension> {
        ...
        std::fs::create_dir_all(&extension_dir)
            .with_context(|| format!("Failed to create extension directory: {:?}", extension_dir))?;
        
        // Load and initialize the WASM module with security constraints
        let mut extension_instance = loader::load_wasm_module(wasm_path, &extension_dir).await
        ...
        let schema_sdl = extension_instance.get_schema()
            .await
